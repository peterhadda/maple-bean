...
let last=0;
renderer.setAnimationLoop(ms=>{const dt=Math.min((ms-last)/1000,.05);last=ms;const still=q().get('still')==='1';maya.update(still?0:ms/1000,dt,{still,expression:expression==='blink'?'neutral':expression,blink:expression==='blink'?1:undefined,walking:pose==='walk',walkDistance:pose==='walk'?dt*1.5:0,sitting:pose==='sit'||pose==='study',studying:pose==='study',wave:pose==='wave',cup:pose==='sip',sipping:pose==='sip',seatHeight});chair.visible=pose==='sit'||pose==='study';controls.update();renderer.render(scene,camera);if(!window.__ready){loading.classList.add('done');document.querySelector('#controls').disabled=false;window.__ready=true;}});
