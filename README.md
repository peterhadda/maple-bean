...
## Unreal Engine 5 edition (in progress)

A separate Unreal Engine 5.6 version lives in `mb_unreal_engine/`. It rebuilds the same café and cast with upgraded visuals, while the browser game stays fully playable on its own.

| Day | Night |
| --- | --- |
| ![Maple Bean in Unreal, daytime overview](docs/unreal/qa/cozy/day-overview.png) | ![Maple Bean in Unreal, evening overview](docs/unreal/qa/cozy/night-overview.png) |
| ![The cast at the coffee bar, day](docs/unreal/qa/cozy/day-characters.png) | ![The cast at the coffee bar, night](docs/unreal/qa/cozy/night-characters.png) |

*Real Unreal renders from the project's QA pipeline (not concept art). Day and night follow the browser game's lighting presets.*

What is in it today:

- **Café:** the full layout from `assets/layout.json`, warm materials (oak, painted wood, fabric, plaster, brass), 40+ real lamp lights, a cozy colour grade, and day and night lighting.
- **Plants and props:** 47 procedural plants (fiddle-leaf fig, monstera, snake plant, pothos, succulents, leafy trees), plus an espresso bar, pastry case, chalk menu, upholstered lounge, study desks, a Quiet Nooks wing, a merch corner, and street furniture.
- **Cast:** Maya, Claire, Noah, Mara and Jules, rebuilt in Blender (fuller hair, softer faces). They keep the 41-bone rig and 30 facial expressions.
- **Gameplay code (C++):** a port of the browser game's walking, seat choreography, regulars' routine, and cameras. It also includes an idle, walk and sit animation setup.

How it is built:

- Everything is generated by scripts: Blender Python for models, and Unreal Python for import, materials, lighting and QA renders. The binary `Content/` folder is not committed; it is rebuilt from `assets/`, `mb_unreal_engine/SourceArt/` and `mb_unreal_engine/Scripts/`.
- Every Unreal run goes through `mb_unreal_engine/Scripts/ue_run.sh`, which allows only one Unreal process at a time. `MB_TIME=day|night Scripts/ue_run.sh <owner> shot <tag>` renders the QA screenshots.
- The target machine is modest: DX11, no ray tracing, and an Intel Iris Xe, so lighting is tuned for integrated graphics.

Status: faces are still the original base with scripted refinements rather than authored sculpts. Animation and the play-mode tests are unfinished. Progress notes are in `docs/unreal/`.
...
| Unreal Engine 5.6 + C++ | Separate high-fidelity edition (`mb_unreal_engine/`) |

The browser game uses Three.js directly and does not need Unreal Engine; the Unreal edition is a separate project.
...
mb_unreal_engine/       Unreal Engine 5 edition (C++ Source, Scripts, Config, SourceArt)
docs/unreal/           Unreal audit, agent notes, targets, QA renders
```
