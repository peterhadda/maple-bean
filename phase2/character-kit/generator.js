...
const paintTex = canvasTex(PS, PS, (g) => {
  ...
  for (const sx of [-1, 1]) {
    ...
    for (let i = 0; i <= M; i++) {
      ...
      const yc = 0.0205 + PAINT.browArch * arch, th = lerp(0.0165, 0.0065, Math.pow(s, 1.3)) * (s < 0.06 ? 0.75 + 0.25 * s / 0.06 : 1) * PAINT.browThickness;
...
if (look.layers.includes('cardigan')) {
  ...
  let sit=0,walk=0,wave=0,sip=0,phase=look.phaseOffset,lastPose='',nextBlink=2.5+look.phaseOffset,blinkStart=-10;
  function update(t=0,dt=.016,state={}) {
    ...
    if(poseKey!==lastPose){
      lastPose=poseKey;
      const pose=poseMatrices({sit,walk,wave,sip,phase:posePhase,time:t,seatHeight,male:look.male});
