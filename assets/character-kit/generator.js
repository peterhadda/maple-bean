...
import { bindBody, applyPose, poseMatrices } from './pose.js';
import { resolveLook, IRIS, BROW, LEGACY_HAIR } from './look.js';
import { paintFor } from './paint.js';
import { BONE_COUNT, bakeSkinAttributes, installSkinning, writeBones } from './skinning.js';
import { HAIR_STYLES } from './hair/styles.js';
import { addNoahOutfit } from './noah-outfit.js';
export function createMaya(options = {}) {
const look = resolveLook(options);
const gameDetail=options.detail==='game';
...
function skinMaterial(paint = null, color = COL.skin) {
  const m = new THREE.MeshPhysicalMaterial({ color, roughness: 0.67, specularIntensity: 0.24,
    sheen: 0.18, sheenRoughness: 0.7, sheenColor: new THREE.Color(0xffc4a8) });
  m.onBeforeCompile = (s) => {
...
function hairMaterial(color = COL.hair) {
  ...
  m.onBeforeCompile = (s) => {
    ...
    s.uniforms.uSpec1.value.multiplyScalar(look.character==='noah'?.32:.65);s.uniforms.uSpec2.value.multiplyScalar(look.character==='noah'?.32:.65);
    s.vertexShader = s.vertexShader
...
const hairMat = look.hair.shader === 'plain' ? new THREE.MeshPhysicalMaterial({color:COL.hair,roughness:.62,vertexColors:true,specularIntensity:.24}) : hairMaterial();
hairMat.envMapIntensity = 0.25; hairMat.specularIntensity = 0.08;

// ------------------------------------------------------------------ root groups
const maya = new THREE.Group(); maya.name = 'Maya';
const HEAD_SCALE = look.male ? 1.07 : 1.14, HEAD_POS = new V3(0, 1.648 - 0.161 * HEAD_SCALE, 0.03);
...
const HR = { x: look.male ? 0.132 : 0.121, y: 0.150, zf: 0.124, zb: 0.108 };   // head topology sheet: ~0.78 wide and ~0.85 deep per unit height
...
const WF = spline1([[0,1],[0.2,1.0],[0.35,0.965],[0.5,0.88],[0.65,0.79],[0.8,0.70],[0.9,0.63],[1, look.male ? 0.78 : 0.58]]);
...
const yL = s => { const a = Math.max(0, 1 - s * s); return cyC(s) - 0.0155 * Math.pow(a, 0.62); };
const mouthSeam = x => -0.0985 + 2.6 * x * x + 0.0009 * clamp(x / 0.034, -1, 1);
...
const MUZZLE = spline1([[-0.16,0],[-0.145,0.008],[-0.132,0.017],[-0.10,0.013],[-0.07,0.010],[-0.045,0.004],[-0.03,0]]);
...
function faceDisplace(p, smile, wink, conform = true) {
  ...
  if (z > 0.01) {
    ...
    const mw = Math.exp(-Math.pow(ax / 0.034, 4));
    z += 0.0038 * mw * gauss(0, y - (seam + 0.0034), 1, 0.0030) * front;
    z += 0.0044 * mw * gauss(0, y - (seam - 0.0054), 1, 0.0038) * front;
    z -= 0.0013 * Math.exp(-Math.pow(ax / 0.037, 6)) * gauss(0, y - seam, 1, 0.0013) * front;
    z -= 0.0013 * gauss(ax - 0.037, y - (seam + 0.002), 0.003, 0.004) * front; // corner dimples
...
const PAINT = paintFor(look, COL.skin, BROW[look.face.brow] ?? look.face.brow);
const paintTex = canvasTex(PS, PS, (g) => {
  ...
  for (const sx of [-1, 1]) {
    ...
    rg.addColorStop(0, PAINT.blush[0]); rg.addColorStop(0.5, PAINT.blush[1]); rg.addColorStop(1, PAINT.blush[2]);
  ...
  rg.addColorStop(0, PAINT.noseFlush[0]); rg.addColorStop(1, PAINT.noseFlush[1]);
  g.fillStyle = rg; g.fillRect(0, 0, PS, PS);
  // freckles
  for (let i = 0; i < PAINT.freckles; i++) {
    const sx = i % 2 ? 1 : -1, fx = sx * (0.035 + R() * 0.06), fy = -0.045 - R() * 0.035;
    g.fillStyle = `rgba(${PAINT.freckle},${0.10 + R() * 0.10})`; g.beginPath(); g.arc(PX(fx), PY(fy), PL(0.0005 + R() * 0.0005), 0, 7); g.fill();
  }
  // nostril shadows
  g.filter = 'blur(2px)';
  for (const sx of [-1, 1]) { g.fillStyle = PAINT.nostril; g.beginPath(); g.ellipse(PX(sx * 0.0078), PY(-0.0852), PL(0.0030), PL(0.0014), sx * 0.35, 0, 7); g.fill(); }
  // lips
  const N = 40, xs = Array.from({length:N+1},(_,i)=>-.034+.068*i/N);
  const top = x => mouthSeam(x) + 0.0050 * Math.pow(Math.max(0, 1 - (x / 0.034) ** 2), 0.8) - 0.0010 * Math.exp(-((x / 0.006) ** 2));
  const bot = x => mouthSeam(x) - 0.0072 * Math.pow(Math.max(0, 1 - (x / 0.0335) ** 2), 0.6);
  g.filter = 'blur(1.6px)';
  ...
  g.fillStyle = PAINT.lipUpper; g.fill();
  ...
  rg.addColorStop(0, PAINT.lipLower[0]); rg.addColorStop(0.5, PAINT.lipLower[1]); rg.addColorStop(1, PAINT.lipLower[2]);
  g.fillStyle = rg; g.fill();
  g.filter = 'blur(0.7px)';
  g.beginPath(); xs.forEach((x, i) => i ? g.lineTo(PX(x), PY(mouthSeam(x))) : g.moveTo(PX(x), PY(mouthSeam(x))));
  g.strokeStyle = PAINT.lipSeam; g.lineWidth = PL(0.0011); g.lineCap = 'round'; g.stroke();
  for (const sx of [-1, 1]) {            // corner flicks
    g.beginPath(); g.moveTo(PX(sx * 0.033), PY(mouthSeam(sx * 0.033)));
    g.quadraticCurveTo(PX(sx * 0.035), PY(mouthSeam(sx * 0.033) + 0.0004), PX(sx * 0.037), PY(mouthSeam(sx * 0.033) + 0.0018));
    g.strokeStyle = PAINT.lipCorner; g.lineWidth = PL(0.0008); g.stroke();
  }
  g.filter = 'blur(2px)'; g.fillStyle = PAINT.lipHighlight;
  ...
  for (const sx of [-1, 1]) {
    ...
    for (let i = 0; i <= M; i++) {
      ...
      const yc = 0.0205 + PAINT.browArch * arch, th = lerp(0.0115, 0.0035, Math.pow(s, 1.3)) * (s < 0.06 ? 0.75 + 0.25 * s / 0.06 : 1) * PAINT.browThickness;
      up.push([x, yc + th * 0.55]); dn.push([x, yc - th * 0.45]);
    }
    g.filter = 'blur(2.2px)'; g.fillStyle = BROW[look.face.brow] ?? look.face.brow;
    ...
    for (let k = 0; k < 320; k++) {
      ...
      g.strokeStyle = R() < 0.5 ? `rgba(${PAINT.browStrokeA},${0.5 + R() * 0.4})` : `rgba(${PAINT.browStrokeB},${0.35 + R() * 0.3})`;
  ...
  for (const sx of [-1, 1]) {
    ...
    g.filter = 'blur(3px)'; g.fillStyle = PAINT.lidShade;
    g.beginPath();
    ...
    g.fill();
    g.filter = 'blur(1.2px)'; g.strokeStyle = PAINT.crease; g.lineWidth = PL(0.0011);
    g.beginPath();
...
function buildHeadGeometry(ROWS = gameDetail?84:116, COLS = gameDetail?112:152) {
  const d = new V3();
...
function eyeTexture() {
  ...
  return canvasTex(W, H, (g) => {
    ...
    const iris = IRIS[look.face.iris] ?? IRIS.brown;
    gr.addColorStop(0, iris.stops[0]); gr.addColorStop(0.18, iris.stops[1]); gr.addColorStop(0.55, iris.stops[2]);
    gr.addColorStop(0.82, iris.stops[3]); gr.addColorStop(1, iris.stops[4]);
    ...
    for (let x = 0; x < W; x++) {
      ...
      g.fillStyle = dn > 0 ? `rgba(${iris.lower},${0.30 * dn})` : `rgba(30,12,5,${0.35 * -dn})`;
...
const eyeMat = new THREE.MeshPhysicalMaterial({ map: eyeTexture(), roughness: 0.35, clearcoat: .7, clearcoatRoughness: 0.10, envMapIntensity: 0.8 });
...
function makeEye(sx) {
  ...
  for (const [ox, oy, r] of [[0.16, 0.17, 0.0038], [-0.13, -0.15, 0.0013]]) {
...
function updateEye(e, blink, squint) {
  ...
  parts.push(taperedTube(line, { segments: 60, radial: 6, radius: t => ((look.male ? 0.0006 : 0.0011) + (look.male ? 0.0008 : 0.0024) * Math.pow(t, 0.9)) * (t > 0.86 ? Math.max(0.25, 1 - (t - 0.86) / 0.14 * 0.75) : 1) * Math.min(1, t * 8 + 0.4), flatten: 0.75 }));
  const spikes = look.male ? [] : [[0.35, 0.0035], [0.52, 0.0048], [0.66, 0.0062], [0.8, 0.0074], [0.93, 0.0082], [1.04, 0.0078], [-0.2, 0.0026], [0.1, 0.003]];
  ...
  e.rim.geometry.dispose(); e.rim.geometry = taperedTube(rim, { segments: 30, radial: 6, radius: t => 0.0007 * Math.pow(Math.sin(Math.PI * t), 0.3) });
}
makeEye(1); makeEye(-1);
...
// legacy base layers (scalp shell, clumps, bangs, temple strands) shared by the canonical styles
if (LEGACY_HAIR.includes(look.hair.style)) {
// scalp shell: grooves converge on the bun
{
  ...
  const d = new V3(), P = new V3();
  const grooveAt = (v) => { const ang = Math.atan2(v.dot(e1), v.dot(e2)); return Math.pow(Math.abs(Math.sin(ang * 11 + .8 * Math.sin(ang * 3))), 0.7); };
  const g = gridSurface(gameDetail?64:90, gameDetail?88:124, (t, u, out) => {
    ...
    const off = lerp(lerp(-0.010, -0.004, ss(0.0, -0.06, P.z)), vol - 0.0013 * grooveAt(n) * pc, Math.pow(h, 1.6)); // eased: hairline feathers instead of forming a rim
  ...
  fillAttr(g, 'color', 3, i => { v.fromBufferAttribute(p, i).normalize(); const k = 0.86 + 0.1 * grooveAt(v); return [k, k * 0.97, k * 0.94]; });
  hairParts.push(prepHair(g));
}
// clumps from hairline (sides/back) to the bun
for (let k = 0; k < 36; k++) {
  const psi0 = (k / 36) * Math.PI * 2 - Math.PI + (rand() - 0.5) * 0.05;
  ...
  const g = taperedTube(pts, { segments: 20, radial: 8, flatten: 0.30, radius: t => 0.018 * Math.min(1, t * 3 + 0.08) * (1 - 0.5 * t) });
...
for (const sd of [-1, 1]) {
  ...
  for (let k = 0; k < n; k++) {
    ...
    const g = taperedTube(pts, { segments: 30, radial: 8, flatten: 0.32, radius: t => (0.015 - 0.003 * q) * Math.min(1, t * 6 + 0.4) * Math.pow(1 - t, 0.8) });
...
// bun: core + twisted rope coils + flyaway loops
{
  ...
  for (let i = 0; i < cp.count; i++) {
    ...
    const gv = Math.pow(Math.abs(Math.sin(lat * 8 + .9 * Math.sin(v.x * 4 + v.z * 3))), 0.6);
    const r = 0.99 + lump - 0.018 * gv;
  ...
  for (let k = 0; k < SUB; k++) {
    ...
    const g = taperedTube(pts.filter((_, i) => i % 3 === 0), { segments: 80, radial: 8, flatten: 0.45, radius: t => rrAt(t) * 0.95 * Math.min(1, (1 - t) * 10 + 0.2) });
  ...
  for (let k = 0; k < 6; k++) {
    ...
    for (const [off, rs] of [[0, 1.4]]) {
      ...
      hairParts.push(prepHair(taperedTube(pts, { flatten: 0.4, radius: strandR(0.013 * rs, 0.5), segments: 36, radial: 8 }), 0.88 + rand() * 0.15));
    }
  }
...
if(look.hair.style==='tousled'){
  hairParts.push(...HAIR_STYLES['textured-quiff']({V3,rand,ss,lerp,dirOf,hairlineElevation,headBase,hairness,shellPoint,gridSurface,taperedTube,prepHair,fillAttr}));
  // Broad swept locks, with loose tips at the temples, from the supplied Noah sheet.
  for(const sd of [-1,1])for(let i=0;i<5;i++){
    const x=sd*(.022+i*.022),y=.14-i*.012;
    const pts=[new V3(x*.3,.16,-.015),new V3(x,y+.025,.09),new V3(x+sd*.026,y-.04,.132),new V3(x+sd*.016,y-.09,.12)];
    hairParts.push(prepHair(taperedTube(pts,{segments:24,radial:10,flatten:.45,radius:t=>.021*Math.pow(1-t,.65)}),.9+i*.025));
  }
}
...
headGroup.add(tie); tie.visible = look.hair.style === 'messy-bun';
if (look.nohair) { hairMesh.visible = false; tie.visible = false; }   // debug: inspect the bare head shape
...
const TOP = { y0: look.male ? 0.97 : 1.030, y1: look.top.neckline === 'cami' ? 1.215 : 1.262 };
...
function torsoPoint(t, phi, out, grow = 0) {
  ...
  if (cs > 0) { const ax = Math.abs(x), sy = y < 1.135 ? 0.021 : 0.03; z += (look.male ? 0.002 : 0.024) * cs * gauss(ax - 0.047, y - 1.135, 0.030, sy); }
...
const topTex = canvasTex(2048, 1024, (g, W, H) => {
  ...
  if (look.top.motif === 'heart') {g.fillStyle = '#ef5a52'; g.fill(); g.lineWidth = 3; g.strokeStyle = 'rgba(190,50,45,0.6)'; g.stroke();} g.restore();
...
addMesh(gridSurface(gameDetail?32:54, gameDetail?64:96, (t, u, v) => torsoPoint(t, (u - 0.5) * 2 * PI, v)), topMat);
...
for (const sd of [-1, 1]) {
  ...
  addMesh(taperedTube([P(0.05, 1.19, -0.02), P(0.095, 1.232, -0.02), P(0.128, 1.222, -0.02), P(0.146, 1.15, -0.014), P(0.160, 1.048, -0.004), P(0.172, 0.97, 0.010), P(0.180, 0.905, 0.020)],
    { segments: 72, radial: 16, radius: t => sleeveR(t) * (1 + 0.035 * Math.sin(t * 60) * ss(0.55, 0.8, t) * ss(1.0, 0.85, t)) }), look.top.sleeves === 'bare' ? skinMat : sleeveMat);
...
addMesh(gridSurface(gameDetail?32:46, gameDetail?80:112, (t, u, v) => hipPoint(t, (u - 0.5) * 2 * PI, v)), hipMat);
...
for (const sd of [-1, 1]) {
  const legGeo = gridSurface(gameDetail?48:80, gameDetail?40:56, (t, u, v) => {
...
if (look.layers.includes('cardigan')) {
  ...
  if(look.character==='noah')addNoahOutfit({THREE,body,addMesh,gridSurface,taperedTube,V3,lerp});
  ...
  if (look.male) for (const entry of bound) {
  ...
  // GPU skinning (opt-in): bake the bind pose (after the masculine widening) and
  // per-vertex influences once; update() then only writes 12 matrices.
  let skin=null;
  if (look.skinning === 'gpu') {
    skin={bones:Array.from({length:BONE_COUNT},()=>new THREE.Matrix4()),pelvis:new THREE.Matrix4()};
    const uniforms={mbBones:{value:skin.bones},mbPelvis:{value:skin.pelvis}};
    const depthMat=installSkinning(new THREE.MeshDepthMaterial({depthPacking:THREE.RGBADepthPacking}),uniforms);
    for (const entry of bound) {
      const g=entry.mesh.geometry;
      g.attributes.position.array.set(entry.positions);g.attributes.position.needsUpdate=true;
      // Batched garment meshes are non-indexed: plain computeVertexNormals faceted every triangle.
      if(look.male)weldNormals(g);
      else g.attributes.normal.array.set(entry.normals);
      const baked=bakeSkinAttributes(entry.positions,entry.region,entry.side);
      g.setAttribute('mbIndex',new THREE.BufferAttribute(baked.index,3));
      g.setAttribute('mbWeight',new THREE.BufferAttribute(baked.weight,3));
      g.setAttribute('mbHip',new THREE.BufferAttribute(baked.hip,1));
      g.boundingSphere=new THREE.Sphere(new V3(0,.9,.05),1.15);
      installSkinning(entry.mesh.material,uniforms);
      entry.mesh.customDepthMaterial=depthMat;
    }
  }
  ...
  let sit=0,walk=0,wave=0,sip=0,study=0,phase=look.phaseOffset,lastPose='',nextBlink=2.5+look.phaseOffset,blinkStart=-10;
  function update(t=0,dt=.016,state={}) {
    ...
    study+=((state.studying&&!state.cup?1:0)-study)*ease;
    ...
    headGroup.rotation.set(.065*study,state.still?0:.018*Math.sin(t*.6),0);
    ...
    maya.position.y=state.still?0:Math.sin(t*1.55+look.phaseOffset)*.006*(1-walk);
    const seatHeight=state.seatHeight??.54;
    const poseKey=[sit.toFixed(3),walk.toFixed(3),wave.toFixed(3),sip.toFixed(3),study.toFixed(3),seatHeight,walk>.002?posePhase.toFixed(3):'',wave>.002||study>.002?t.toFixed(3):''].join(':');
    if(poseKey!==lastPose){
      lastPose=poseKey;
      const pose=poseMatrices({sit,walk,wave,sip,study,phase:posePhase,time:t,seatHeight,male:look.male});
      if(skin)writeBones(pose,skin.bones,skin.pelvis);else applyPose(bound,pose);
      headGroup.position.y=HEAD_POS.y-pose.drop;
      cup.position.set(look.male?.22:.192,.75,.07).applyMatrix4(pose.arms[1].lower);cup.rotation.x=-.4*sip;
    }
  }
  update(0,0,{still:true});
  maya.name=look.name||'Maya';
  return {group:maya,update,head:headGroup,body,face,hair:hairMesh,skinning:skin?'gpu':'cpu'};
}
