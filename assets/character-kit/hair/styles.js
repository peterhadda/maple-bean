...
function texturedQuiff(k) {
  ...
  parts.push(scalpShell(k, {
    volume: P => 0.006 + 0.014 * ss(0.0, 0.12, P.y) + 0.006 * ss(0.02, 0.11, P.z) * ss(0.02, 0.12, P.y),
    flow: (v, T) => T.set(0.3 * ss(0.02, 0.12, v.y), -0.45 * (1 - ss(0.0, 0.09, v.y)), -1),
    tint: v => 0.88 + 0.04 * Math.sin(v.x * 70 + v.z * 45) * Math.sin(v.y * 60 + v.x * 20),
  }));
  const SWEEP = 0.26;
  // Quiff and top: rows of ribbons from the front hairline back over the crown.
  for (let row = 0; row < 3; row++) for (let c = 0; c < 7; c++) {
    const psi0 = lerp(-0.9, 0.9, c / 6) + (rand() - 0.5) * 0.07;
    ...
    const R = (0.021 - 0.003 * row / 3) * (0.9 + 0.2 * rand());
    parts.push(ribbon(k, d0, d1, {
      ...
      flatten: 0.38, tint: 0.88 + rand() * 0.15,
    }));
  }
