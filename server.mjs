...
const snapshot=()=>[...peers.values()].map(({token,lastChat,...p})=>p);
  ...
  try {
    ...
    if(req.method==='POST'&&url.pathname==='/api'){
      ...
      if(data.action==='join'){
        ...
        const p={id:randomUUID(),token:randomUUID(),name,x:0,z:5.6,angle:Math.PI,seatId:null,lastChat:0};peers.set(p.id,p);
      ...
      if(data.action==='rename'){
        ...
        const s=layout.stations.find(s=>s.id===data.seatId&&['seat','read'].includes(s.kind));
...
setInterval(()=>{for(const c of clients)c.res.write(': heartbeat\n\n');},15000).unref();
