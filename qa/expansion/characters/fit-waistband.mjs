// Settle the existing male trouser waistband against the torso; preserve shirt and leg shape.
import fs from 'node:fs';const smooth=(a,b,v)=>{let t=Math.max(0,Math.min(1,(v-a)/(b-a)));return t*t*(3-2*t)};
fs.mkdirSync('qa/expansion/characters/waistband-source',{recursive:true});
for(const id of ['jules','noah'])for(const suffix of ['','-lod']){
 const file=`assets/characters/${id}${suffix}.glb`,src=`qa/expansion/characters/waistband-source/${id}${suffix}.glb`;if(!fs.existsSync(src))fs.copyFileSync(file,src);const bytes=fs.readFileSync(src),n=bytes.readUInt32LE(12),j=JSON.parse(bytes.subarray(20,20+n)),bin=bytes.subarray(n+28),seen=new Set();let count=0;
 ...
 for(let i=0;i<positions.length;i++){const g=groups.get(find(i)),belt=['Wardrobe','Accessories'].includes(mat)&&g.max<1.04&&g.max>.985;
 const deform=([x,y,z])=>{const fit=belt?smooth(.94,1.0,y):0;return[x*(1-.10*fit),y,z*(1-.12*fit)]};
