...
// The hosted edition has no second guest, so its header badge says so. Rebuilding
// used to copy the local page verbatim and quietly relabel it "LOCAL PLAYTEST".
for(const name of fs.readdirSync(dest).filter(n=>n.endsWith('.html'))){const p=path.join(dest,name);fs.writeFileSync(p,fs.readFileSync(p,'utf8').replaceAll('/node_modules/three/','/vendor/three/').replaceAll('LOCAL PLAYTEST','SOLO CAFÉ'));}
